...
class Misparticipantes extends Controller
{
        ...
        try {
                ...
                if ($cartaHoy) {
                  ...
                  return response()->json(['ok'=>  $ninio]);
                    ...
                    return response()->json(['msj'=>"Advertencia! no Puede crear más cartas en esta fecha ya que la fecha esta en estado Enviado"]);
  ...
  public function enviarCarta(Request $request)
    ...
    public function envioCorreos($user,$buzon)
    {
      ...
      $msg_was='Estimadx Niñx '.$buzon->ninio->nombres. ' Ingresa al link tienes cartas que revisar. '.
      'Ingresar a mi buzon de mensajes '. route('entrada',$buzon->ninio->token);
      ...
      if($user->familia->otro2){
        try {
          ...
          $data_r = [
              'phone' => $user->familia->otro2, 
              'body' => $msg_was,
          ];
          $json_r = json_encode($data_r);

          $options_r = stream_context_create(['http' => [
                  'method'  => 'POST',
          ...
          file_get_contents($url_was_api, false, $options_r);
        } catch (\Throwable $th) {
